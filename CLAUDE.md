...
- "make the podcast for X" / "add a podcast episode for X" / "make an audio episode for X" / "narrate X" → **Procedure A, podcast step** — produce & link one podcast episode for an **existing** explainer (the paper must already be a node). This command is for producing an episode *on its own*; the same step also runs automatically as the last step of every paper (see **Podcast is mandatory** below).
...
**It is a queue, not a stack.** Papers discovered while processing another paper are **appended at the bottom**, never pushed onto the top. Pushing newly-discovered papers to the top turns the pipeline into a depth-first crawl that tunnels into whatever sub-topic the last paper cited and starves the high-value seeds that motivated the search — a real failure mode, observed once (four papers in, the run had burrowed into theory-of-mind benchmarks while PlanBench, AgentClinic, TaskBench and BrowserGym sat untouched).

**Reprioritize periodically** rather than trusting insertion order forever: `python3 scripts/reprioritize_queue.py <survey_id>` re-sorts a survey's `core` entries by a score that favours (a) the benchmark/eval space and (b) high citation counts, backfilling missing `citation_count`s from Semantic Scholar first. Re-run it every so often — after a burst of papers, or whenever the head of the queue stops looking like the most valuable work available. It only reorders; it never adds, drops, or edits entries.
...
`audio_url` is **required on every node** and goes **right after `explainer`** (before `tags`): a paper is not done until its podcast episode exists, so a node without one is unfinished work, not a normal state. It must be the canonical published URL for the slug: `https://pod.darvinyi.com/audio/<slug>.mp3` (the episode is produced in the separate `darvinyi-podcast` repo; see the mandatory podcast step in Procedure A). `python3 scripts/litsearch.py complete-paper` refuses to mark a paper done while this key is missing. Don't hand-write it — `python scripts/inject_podcast.py --set <slug>` stamps it deterministically (canonical key order preserved) **and** injects the on-page player; `test_data_integrity.py` enforces the canonical value.
...
- **When the arXiv preprint and the published version materially differ, build from the published version and label every divergence.** The arXiv link stays (it carries the `date`), so a reader clicking `source ↗` may land on a *different paper* than the page describes. Established on TAPE (`rao-2019-tape`), where arXiv hosts **only v1** while the NeurIPS camera-ready has different Heldout-Families numbers (Transformer 0.30/13.04/10.04 → 0.35/11.77/8.87), an extra table column-group, a GLUE citation v1 lacks, **no appendix at all**, and a renumbered supplement (v1's Table S8 is the camera-ready's S9). The pattern that worked there, and the one to copy:
  1. a short **version-note callout** near the top of the method section, placed **before the first divergent number**, saying which version the page follows and what `source ↗` actually resolves to;
  2. every divergent value **scoped inline** — print the published number and state the preprint's alongside it;
  3. every appendix citation **labelled with the version whose numbering it uses**, since supplements get renumbered and a published version may carry no appendix at all.
  Check for this whenever S2's `publicationDate` and venue disagree with the staged PDF, or the venue is a preprint server (S2 reported TAPE's venue as `bioRxiv` for a NeurIPS paper).
  - **The published version is not automatically the authoritative one.** Build from whichever version the *authors* endorse as current. On FRANK (`pagnoni-2021-frank`) the arXiv comment reads "Accepted at NAACL 2021. **Second version fixes bug with BERTScore results**", and the ACL Anthology copy still carries the bug: its Table 2 gives BERTScore P as &minus;0.02 / &minus;0.01 / 0.00, all insignificant (p = 0.35, 0.69, 0.95), where arXiv v2 gives 0.24 / 0.35 / 0.29 at p&lt;0.01 &mdash; and the two versions' prose disagree about which metrics lead. That is the reverse of TAPE. **Read the arXiv comment field** (`https://arxiv.org/abs/<id>`, "Comments:") before choosing; it is where authors announce corrections, and it takes one request. Scope the divergence on the page either way, and say which version you followed and why.
...
**Mandatory — podcast episode (every paper, always).** **A paper is not done until its TTS episode is published and linked.** Producing one is expensive (a large LLM script pass + ~18 min TTS render), but it is **not optional and not skippable**: it is the last step of every paper, and `scripts/litsearch.py complete-paper` enforces it (it refuses unless `current.step` is `podcast` *and* the node carries an `audio_url`). This is also the whole of the standalone "make the podcast for `<slug>`" command. The paper must already be a node (do Procedure A first). Then:
1. **Produce it in the separate podcast repo.** `cd ~/Projects/darvinyi-podcast` and run the user-level `litsearch-podcast` skill for the slug. It reads **this** repo's `public/papers/<slug>.html` (its `litsearch.repo`/`content_hint` point here) plus the paper PDF, writes a 3-voice script to `scripts/<slug>.md`, renders the MP3 with Kokoro, publishes it to the NAS, and rebuilds the RSS feed. Integration is via the shared slug **only** — never merge the podcast repo into this one.
   - **The Kokoro render must be detached with `nohup`, not left in a background shell.** An ~18-minute render launched under a background-shell wrapper gets reaped when that shell is torn down, leaving a zero-byte log, no MP3, and no error — the script step looks like it succeeded and the failure is silent. `setsid` does **not** exist on macOS; use `nohup` inside a detached subshell. Observed once on `dziri-2022-begin`, where a monitor sat waiting on a process that had already been killed.
   - **Confirm the artifact, never the watcher.** Check that `audio/<slug>.mp3` exists at a plausible size (recent episodes run ~21–27 MB for 25–33 min) before moving on. A monitor armed over a dead render reports nothing forever; `ls` and `curl -sI` are the source of truth.
   - **Gate on the render's exit code, not on the file.** Kokoro's write behaviour differs between runs: on `hu-2020-xtreme` it created the MP3 as a **0-byte file** and streamed ~21 MB into it over about a minute, so `until [ -f <mp3> ]` false-positives on an empty file; on `hu-2023-tifa` it wrote nothing until the very end, so the same check would have hung instead. Both were observed within a few papers of each other, so neither file-based test is reliable on its own. Write an exit-code sentinel from the detached subshell and poll that; use `[ -s <mp3> ]` as a secondary check and settle it with `ls -la` / `ffprobe` / MD5.
...
- **Podcast is mandatory, expensive, and lives in a separate repo.** **A paper is not done until its TTS episode is done** — every node carries an `audio_url` (`https://pod.darvinyi.com/audio/<slug>.mp3`), and `scripts/litsearch.py complete-paper` refuses to close a paper without one. The on-page `listen ♪` pill + `<audio>` player are owned by `scripts/inject_podcast.py` (never hand-edit them). The episode itself is built in `~/Projects/darvinyi-podcast` (the user-level `litsearch-podcast` skill), integrated here **through the shared slug only** — never merge the repos. The expense is a reason to budget for it, never a reason to defer it: shipping an explainer without its episode leaves the paper unfinished.
